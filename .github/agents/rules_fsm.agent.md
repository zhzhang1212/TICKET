...
1. 订单生命周期 (FSM)：
   - 订单包含 待支付/确认、已确认、已取消、已爽约 等状态。
   - 竞态条件防范：必须处理“幽灵支付”问题。例如用户在 15 分钟倒计时最后一秒完成支付，而系统恰好触发超时取消逻辑，需利用数据库乐观锁（Optimistic Locking）和延迟消息队列（Delayed MQ）保障状态绝对一致。

2. 动态预约规则引擎：
   - 废弃在核心代码中堆砌 if-else 的做法。针对多维规则（如：“本科生限额 2 小时”、“信用分低于 80 额度减半”、“爽约扣减 10 分”），需实现责任链模式（Chain of Responsibility）或表达式引擎，确保规则的可配置与高扩展性。